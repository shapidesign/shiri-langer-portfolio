...
export class DragManager {
  ...
  constructor() {
    ...
    this.config = {
      friction: 0.95, // More friction for faster stop
      minSpeed: 0.2, // Higher minimum speed to stop sooner
      maxVelocity: 20, // Higher max velocity
      resistance: 1.0
    };
  }
  ...
  public initialize(): void {
    ...
    if (this.isMobile) {
      this.config = {
        friction: 0.92, // More friction for mobile
        minSpeed: 0.15, // Higher minimum speed for mobile
        maxVelocity: 15, // Higher max velocity for mobile
        resistance: 0.9 // More resistance for mobile
      };
    }
  }
  ...
  private handlePointerMove = (e: React.PointerEvent, setOffset: React.Dispatch<React.SetStateAction<DragOffset>>): void => {
    ...
    const velocityMultiplier = this.isMobile ? 12 : 15; // Reduced for smoother feel
