...
export class ProjectService {
  ...
  private generateProjects(): void {
    ...
    for (const projectText of PROJECT_TEXTS) {
      const project: Project = {
        ...
        imagePosition: projectText.imagePosition,
  ...
  // Fixed layout for row 0: Tomi centered at col 0, specific projects on each side
  private readonly STATIC_LAYOUT: Record<string, number> = {
    '0,-7': 13, // Dancing Pot
    '0,-6': 15, // Coffee Machine
    '0,-5': 14, // Tambourine
    '0,-4': 11, // K-SENSE
    '0,-3': 8,  // Mico
    '0,-2': 1,  // Red Chair
    '0,-1': 2,  // 3D Filters (camera)
    '0,0':  0,  // Tomi
    '0,1':  6,  // Rocky Stool
    '0,2':  3,  // PITA
    '0,3':  4,  // Project Itamar
    '0,4':  10, // EVA
    '0,5':  5,  // Lamp Design
    '0,6':  12, // Ember
  };
  ...
  /**
   * Get project index for grid position
   * Ensures no adjacent tiles have the same project by checking neighbors
   */
  public getProjectIndex(row: number, col: number): number {
    const key = `${row},${col}`;
    
    // Check static layout map first (row 0 fixed arrangement)
    if (key in this.STATIC_LAYOUT) {
      const idx = this.STATIC_LAYOUT[key];
      this.projectCache.set(key, idx);
      return idx;
    }
    ...
    // Generate a candidate project index
    const seed = row * 1000 + col;
    let candidate = Math.abs(seed) % this.projects.length;
    
    // Check adjacent positions to avoid duplicates
    ...
    // Get adjacent project indices
    const adjacentIndices = adjacentPositions
      .map(pos => this.projectCache.get(pos))
      .filter(idx => idx !== undefined) as number[];
    
    // If candidate is same as adjacent, pick a different one
    if (adjacentIndices.includes(candidate)) {
      // Find a different project index
      for (let i = 0; i < this.projects.length; i++) {
        const testIdx = (candidate + i + 1) % this.projects.length;
        if (!adjacentIndices.includes(testIdx)) {
          candidate = testIdx;
          break;
        }
      }
    ...
    // Cache the result
    this.projectCache.set(key, candidate);
