...
const PortfolioGrid: React.FC = () => {
  ...
  // Compute initial offset so Tomi (col 0 in STATIC_LAYOUT) is centered on screen
  const initialOffsetX = useMemo(() => {
    const stride = gridConfig.tileWidth + gridConfig.tileGap;
    return (gridConfig.visibleCols * stride) / 2 - gridConfig.tileWidth / 2;
  }, [gridConfig]);

  // Use custom hooks for functionality
  const { offset, setOffset, onPointerDown, onPointerMove, onPointerUp } = useDragInertia(initialOffsetX);
